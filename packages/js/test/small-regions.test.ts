...
describe("enlargeSmallParts", () => {
  it("gives every part of a wholly small feature a visible minimum extent", () => {
    ...
    expect(bigWidth).toBeCloseTo(8, 5);
    expect(smallWidth).toBeCloseTo(8, 5);
  });
  ...
  it("caps growth, so a speck never reads as a real landmass", () => {
    ...
    const [ring] = enlargeSmallParts([square(0, 0, 1)], 800);
  ...
  it("scales each part independently, keeping them apart", () => {
    ...
    const first = boundsOfRing(rings[0]!);
    const second = boundsOfRing(rings[1]!);
    expect((first[0] + first[2]) / 2).toBeCloseTo(.5, 5);
    expect((second[0] + second[2]) / 2).toBeCloseTo(100.5, 5);
  });
});
