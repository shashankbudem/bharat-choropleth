...
class _IndiaChoroplethState extends State<IndiaChoropleth> {
  ...
  (List<ChoroplethRegion>, List<ChoroplethReferenceRegion>) _project(
    ...
    for (final feature in features) {
      ...
      final projectedRings =
          keepsTrueGeometry(feature.id) ? rings : enlargeSmallParts(rings, widget.minPartExtent);
  ...
  void _prepare() {
    final (stateRegions, stateReferences) = _project(widget.features, widget.values, widget.referenceOverlay);
    ...
    if (_drillDownId != null && districts != null) {
      ...
      final (regions, references) = _project(districts.features, districts.values, overlay);
      _regions = regions;
