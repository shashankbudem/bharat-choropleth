...
class MercatorProjection {
  ...
  factory MercatorProjection.fit(
    ...
    final scale = math.min(scaleX, scaleY).isFinite ? math.min(scaleX, scaleY) : 1.0;
...
Offset labelPointFor(List<List<Offset>> rings, Rect bounds) {
  ...
  return centroid.dx.isFinite && centroid.dy.isFinite ? centroid : bounds.center;
}
...
class ViewBoxFit {
  ...
  Offset toViewBox(Offset local) => Offset((local.dx - dx) / scale, (local.dy - dy) / scale);

  Rect get destination => Rect.fromLTWH(dx, dy, kViewBox.width * scale, kViewBox.height * scale);
}
...
List<Offset> convexHull(List<Offset> points) {
  final sorted = [...points]..sort((a, b) => a.dx != b.dx ? a.dx.compareTo(b.dx) : a.dy.compareTo(b.dy));
  final unique = <Offset>[];
  ...
  List<Offset> half(List<Offset> ordered) {
    ...
    for (final point in ordered) {
      while (chain.length >= 2 && turn(chain[chain.length - 2], chain.last, point) <= 0) {
        chain.removeLast();
      }
      chain.add(point);
    }
...
///    turned coastlines into caricatures. Both read as obviously fake.
///
...
List<List<Offset>> enlargeSmallParts(
  ...
  final scale = math.min(minExtent / largest, maxScale);
  return rings.map((ring) {
    final centre = boundsOfRing(ring).center;
    return ring
        .map((point) => Offset(
