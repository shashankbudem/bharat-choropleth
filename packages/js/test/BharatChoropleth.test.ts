...
describe("BharatChoropleth with fetched geometry", () => {
  ...
  it("uses a parent value for a singleton district with the same label", async () => {
    const lakshadweepState: GeometrySource = {
      type: "FeatureCollection",
      features: [{ type: "Feature", properties: { id: "in-cs-31-lakshadweep", name: "Lakshadweep" }, geometry: geometry.type === "FeatureCollection" ? geometry.features[0]!.geometry : (geometry as never) }],
    };
    const lakshadweepDistrict: GeometrySource = {
      type: "FeatureCollection",
      features: [{ type: "Feature", properties: { id: "in-cd-31-342", name: "Lakshadweep" }, geometry: geometry.type === "FeatureCollection" ? geometry.features[0]!.geometry : (geometry as never) }],
    };
    vi.spyOn(globalThis, "fetch")
      .mockResolvedValueOnce({ ok: true, status: 200, json: async () => lakshadweepState } as Response)
      .mockResolvedValueOnce({ ok: true, status: 200, json: async () => lakshadweepDistrict } as Response);

    const map = new BharatChoropleth(container, { values: { Lakshadweep: 43 } });
    await map.ready;
    map.drillDown("Lakshadweep");
    await new Promise((resolve) => setTimeout(resolve, 0));

    expect(regionText(container, /^Lakshadweep,/)).toMatch(/43/);
    map.destroy();
  });
