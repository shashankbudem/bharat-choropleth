...
import { bandOf, colorOf, FEEDS, RAMP, rollUp, seed, STATES, tick, type Values } from "./feeds.ts";

const TICKS = 200;

// Exactly one tile claims absolute bands; the rest are deliberately relative.
assert.equal(FEEDS.filter((feed) => feed.breaks).length, 1, "expected one fixed-band feed");

for (const feed of FEEDS.filter((feed) => feed.breaks)) {
  // Fixed bands are only an absolute reading if they are ordered and total: every
  // finite number lands in exactly one band, and a bigger number never lands lower.
  assert.deepEqual(
    [...(feed.breaks as number[])].sort((a, b) => a - b),
    [...(feed.breaks as number[])],
    `${feed.id}: thresholds are not ascending`,
  );
  assert.equal(bandOf(feed, null), null, `${feed.id}: no data should have no band`);
  assert.equal(colorOf(feed, null), "var(--india-map-empty)", `${feed.id}: no data should be the empty fill`);

  const breaks = feed.breaks as [number, number, number, number];
  const probes = [-1e6, 0, ...breaks.flatMap((b) => [b - 1e-9, b, b + 1e-9]), 1e9];
  let lowest = -1;
  for (const probe of probes) {
    const band = bandOf(feed, probe) as number;
    assert.ok(band >= 0 && band < RAMP.length, `${feed.id}: ${probe} fell outside the ramp`);
    assert.ok(band >= lowest, `${feed.id}: ${probe} banded lower than a smaller reading`);
    lowest = band;
  }
  // The point of the exercise: a state's fill must not depend on the other 35.
  assert.equal(
    colorOf(feed, (feed.breaks as number[])[0] as number),
    colorOf(feed, (feed.breaks as number[])[0] as number),
    `${feed.id}: fill is not a pure function of the reading`,
  );
  assert.notEqual(
    colorOf(feed, ((feed.breaks as number[])[0] as number) - 1e-9),
    colorOf(feed, (feed.breaks as number[])[0] as number),
    `${feed.id}: the first threshold does not change the fill`,
  );
}
