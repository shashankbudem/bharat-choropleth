...
export interface IndiaChoroplethOptions {
  ...
  /** Controlled state drill-down. Use null for the state map. Omit to stay uncontrolled. */
   ...
   * one. The renderer also clears its own uncontrolled selection on such a
  ...
  renderTooltip?: (context: TooltipContext) => DomRenderResult;
  renderInsights?: (context: InsightContext | null, container: HTMLElement) => void;
  /**
  ...
  /**
   * Like minPartExtent, but applied only after drilling into a state's districts.
