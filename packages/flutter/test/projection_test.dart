...
MapFeature box(String id, double west, double south, double east, double north) => MapFeature(
      id: id,
      name: id,
      rings: [
        [
...
Iterable<Offset> projectAll(MercatorProjection projection, List<MapFeature> features) sync* {
...
void main() {
  group('MercatorProjection.fit', () {
    test('keeps every projected point inside the view box, padding included', () {
      ...
      for (final point in projectAll(projection, features)) {
        ...
        expect(point.dx, lessThanOrEqualTo(kViewBox.width - kViewBoxPadding + 0.001));
        expect(point.dy, greaterThanOrEqualTo(kViewBoxPadding - 0.001));
        expect(point.dy, lessThanOrEqualTo(kViewBox.height - kViewBoxPadding + 0.001));
      }
    });
    ...
    test('throws on empty geometry rather than painting a blank map', () {
      ...
      expect(
        () => MercatorProjection.fit([const MapFeature(id: 'a', name: 'a', rings: [])]),
        throwsArgumentError,
      );
    });
  });
  ...
  group('ViewBoxFit', () {
    ...
    test('toViewBox is the exact inverse of what the painter applies', () {
      ...
      final local = Offset(viewBoxPoint.dx * fit.scale + fit.dx, viewBoxPoint.dy * fit.scale + fit.dy);
  ...
  group('enlargeSmallParts', () {
    ...
    test('preserves relative sizes within a feature', () {
      // Andaman & Nicobar: scaling each island up to the threshold on its own
      // made all fifteen exactly the same size, which is what read as fake.
      ...
      expect(big / small, closeTo(4, 0.0001)); // 4:1 before, 4:1 after
    ...
    test('keeps each part''s shape, scaling both axes by the same factor', () {
      ...
      expect(bounds.width / bounds.height, closeTo(4, 0.0001)); // aspect preserved
      expect(bounds.width, closeTo(8, 0.0001));
    });

    test('scales each part independently, keeping them apart', () {
      final rings = enlargeSmallParts([square(0, 0, 1), square(100, 0, 1)], 6);
      expect(boundsOfRing(rings[0]).left, lessThan(boundsOfRing(rings[1]).left));
    });
  });
  ...
  group('scatteredHitArea', () {
    ...
    test('spans the water an island group encloses', () {
      ...
      final hull = scatteredHitArea([square(0, 0, 1), square(8, 20, 1), square(2, 40, 1)], 22)!;
    ...
    test('leaves a region that is already easy to aim at', () {
      expect(scatteredHitArea([square(0, 0, 30), square(0, 60, 30)], 22), isNull);
    });
  });
