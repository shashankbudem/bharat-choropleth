...
4. **Quick start** — the zero-config `BharatChoropleth` with state values, then a pointer to the controlled `IndiaChoropleth` example for callers who own their geometry.
