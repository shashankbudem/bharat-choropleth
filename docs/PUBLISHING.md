...
Versions differ per package and are meant to: `bharat-choropleth` and `bharat-choropleth-js` are at `0.3.1` on npm (2026-09-13), while `bharat_choropleth` on pub.dev and `bharat-choropleth` on PyPI remain at `0.3.0`, which is the last release that changed them. Earlier lockstep was a coincidence of releasing together, not a rule — do not bump a package that has no changes just to make the numbers match. `pnpm verify:data-pin` passes against the `v0.3.0` tag, re-checked 2026-09-13 after the `0.3.1` npm release. `DEFAULT_DATA_BASE_URL` is deliberately still pinned to `v0.3.0`: `0.3.1` changed code, not geometry, so that tag still serves every level, and moving a pin that works would mean another release to no purpose. Move it when the generated data changes — and verify before publishing, not after. Verify the intended version against the registry rather than this sentence — it has been stale before, and was again between the 0.2.0 release and this line being corrected. Before every release, verify that the intended version is not already published and update the root README and package links only after the registry confirms it.
